...
import './draggableBlob.css'

class DraggableBlob extends React.Component {
  constructor(props) {
    super(props)
    this.state = {}
  }
  componentDidMount() {
    ...
    function BlobWorld() {
      ...
      function mergeBlobs(blobA, blobB) {
        ...
        if (
          t - blobs[blobA].lastSplitTime > 500 &&
          t - blobs[blobB].lastSplitTime > 500
        ) {
      ...
      function loop() {
        ...
        for (u1 = 0, ulen = blobs.length; u1 < ulen; u1++) {
          ...
          context.clearRect(
            blob.dirtyRegion.left - dirtySpread,
            blob.dirtyRegion.top - dirtySpread,
            blob.dirtyRegion.right - blob.dirtyRegion.left + dirtySpread * 2,
            blob.dirtyRegion.bottom - blob.dirtyRegion.top + dirtySpread * 2
          )
        ...
        if (dragBlob) {
          dragBlob.velocity.x +=
            (mouseX + mouseDownOffset.x - dragBlob.position.x) * 0.01
          dragBlob.velocity.y +=
            (mouseY + mouseDownOffset.y - dragBlob.position.y) * 0.01
        }
        ...
        for (u1 = 0, ulen = blobs.length; u1 < ulen; u1++) {
          ...
          for (u2 = 0; u2 < ulen; u2++) {
            ...
            if (otherBlob != blob) {
              var distance = distanceBetween(
                ...
                {x: otherBlob.position.x, y: otherBlob.position.y}
              )
          ...
          blob.velocity.x +=
            (window.screenX - screenX) * (0.04 + Math.random() * 0.1)
          blob.velocity.y +=
            (window.screenY - screenY) * (0.04 + Math.random() * 0.1)
          ...
          if (dragNode) {
            var angle = Math.atan2(
              mouseY - (blob.position.y - 80),
              mouseX - blob.position.x
            )
          ...
          for (i = 0, len = blob.nodes.length; i < len; i++) {
            ...
            for (j = 0; j < node.joints.length; j++) {
              ...
              var strainX =
                joint.node.ghost.x -
                node.ghost.x -
                (joint.node.normal.x - node.normal.x)
              var strainY =
                joint.node.ghost.y -
                node.ghost.y -
                (joint.node.normal.y - node.normal.y)
            ...
            node.position.x = Math.max(
              Math.min(node.position.x, worldRect.x + worldRect.width),
              worldRect.x
            )
            node.position.y = Math.max(
              Math.min(node.position.y, worldRect.y + worldRect.height),
              worldRect.y
            )

            // Expand the dirty rect if needed
            blob.dirtyRegion.left = Math.min(
              blob.dirtyRegion.left,
              node.position.x
            )
            blob.dirtyRegion.top = Math.min(
              blob.dirtyRegion.top,
              node.position.y
            )
            blob.dirtyRegion.right = Math.max(
              blob.dirtyRegion.right,
              node.position.x
            )
            blob.dirtyRegion.bottom = Math.max(
              blob.dirtyRegion.bottom,
              node.position.y
            )
          }
          ...
          context.moveTo(
            cn.position.x + (nn.position.x - cn.position.x) / 2,
            cn.position.y + (nn.position.y - cn.position.y) / 2
          )
...
export default connect(mapState, mapDispatch)(DraggableBlob)
